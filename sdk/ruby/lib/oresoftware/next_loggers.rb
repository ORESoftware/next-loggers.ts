...
module ORESoftware
  ...
  module NextLoggers
    ...
    class Logger
      ...
      def with_otel(enabled) = set_otel_enabled(enabled)

      def otel_transport?(transport)
        name = transport.respond_to?(:name) ? transport.name.to_s.downcase : ""
        (transport.respond_to?(:otel?) && transport.otel?) || name == "opentelemetry"
      end
      ...
      def emit_event(event)
        ...
        transports_for(event.otel_enabled?(@otel_enabled)).each do |transport|
