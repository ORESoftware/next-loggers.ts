...
final class ShutdownTrigger {
  const ShutdownTriggger(this.name);
  final String name;
...
final class ShutdownCoordinator {
  ShutdownCoordinator({
    ...
    if (gracePeriod.isNegative) {
      throw ArgumentError.value(gracePeriod, 'gracePeriod', 'must be non-negative');
    }
  }
  ...
  void _transition(
    ...
    try {
      final observed = _onEvent?.call(ShutdownEvent(
        phase: next,
  ...
  void _beginDrain(ShutdownTriggger trigger, bool interactive) {
    ...
    Future<void>.sync(() => _drain(_context(trigger, interactive))).then((_) {
      ...
      _errors.add(error);
      if (_phase == ShutdownPhase.draining) {
        _transition(ShutdownPhase.draining, ShutdownTrigger.drainError, interactive, error);
  ...
  void _beginForce(ShutdownTrigger trigger, bool interactive) {
    if (_phase == ShutdownPhase.forcing || _phase == ShutdownPhase.stopped) return;
    _forced = true;
    _timer?.cancel();
  ...
  Future<void> _finish(
    ...
    if (_completion.isCompleted) return;
    _completion.complete(ShutdownResult(
      forced: _forced,
