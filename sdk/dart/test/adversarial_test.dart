...
void main() {
  group('Zone context', () {
    ...
    test(
      '100 concurrent async operations never cross-contaminate traces',
      () async {
    ...
    test(
      'explicit trace remains primary while ambient trace is retained',
      () async {
        final memory = MemoryTransport();
  ...
  group('logger lifecycle', () {
    test('minimum level filters records before transports', () async {
      ...
      expect(memory.records.map((record) => record.level), [
        LogLevel.warn,
        LogLevel.error,
        LogLevel.fatal,
      ]);
    ...
    test(
      'send is idempotent and returns the cached immutable record',
      () async {
        final memory = MemoryTransport();
        final logger = Logger(console: false, transports: [memory]);
    ...
    test(
      'transport failures are aggregated after all transports receive data',
      () async {
        final memory = MemoryTransport();
    ...
    test(
      'wire JSON omits empty optionals and escapes control characters',
      () async {
        final memory = MemoryTransport();
        final logger = Logger(
          appName: 'wire',
  ...
  group('Supabase transport', () {
    test(
      'batches in insertion order and serializes the shared wire schema',
      () async {
        ...
        expect(batches.single.map((record) => record['message']), [
          'one',
          'two',
          'three',
        ]);
    ...
    test(
      'failed batches are requeued without reordering or data loss',
      () async {
  ...
  group('explicit OpenTelemetry bridge', () {
    test(
      'successful lifecycle propagates Zone context through async work',
      () async {
        final memory = MemoryTransport();
        final logger = Logger(
    ...
    test(
      'callback error identity and stack are preserved after reporting',
      () async {
        final memory = MemoryTransport();
        final logger = Logger(
          minimumLevel: LogLevel.debug,
        ...
        try {
          await withOtelSpan<int>(logger, _TestTracer(span), 'failure', (
            _,
          ) async {
            await Future<void>.delayed(Duration.zero);
            throw expected;
    ...
    test(
      'start, status, record, and end failures never replace app results',
      () async {
        final memory = MemoryTransport();
        final logger = Logger(
          minimumLevel: LogLevel.debug,
