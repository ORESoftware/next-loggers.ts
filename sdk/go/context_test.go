...
func TestMergeAndApplyContext(t *testing.T) {
	...
	if !ok { t.Fatal("missing merged context") }
	if merged.LoggedInUser["id"] != "user-1" || merged.LoggedInUser["role"] != "admin" { t.Fatalf("bad user merge: %#v", merged.LoggedInUser) }
	if len(merged.Users) != 2 || merged.Fields["request"] != "inner" || merged.Fields["keep"] != true { t.Fatalf("bad collection merge: %#v", merged) }
	if merged.TraceID != "trace-inner" || len(merged.TraceIDs) != 2 || len(merged.Tags) != 2 { t.Fatalf("bad trace/tag merge: %#v", merged) }
	logger, transport := contextLogger()
	if err := logger.InfoContext(ctx, "inside").Send(); err != nil { t.Fatal(err) }
	record := transport.Records[0]
	if record.TraceID != "trace-inner" || record.Fields["otel.span_id"] != "span-1" || record.RoutineID != "checkout" { t.Fatalf("context not applied: %#v", record) }
	if record.LoggedInUser["role"] != "admin" || len(record.Users) != 2 { t.Fatalf("users not applied: %#v", record) }
	merged.Fields["request"] = "mutated"
	again, _ := LogContextFrom(ctx)
	if again.Fields["request"] != "inner" { t.Fatal("context snapshot leaked mutable state") }
}
...
func TestContextIsolatedAcrossGoroutines(t *testing.T) {
	...
	for index := 0; index < count; index++ {
		...
		go func(index int) {
			...
			if !ok || value.TraceID != expected || value.Fields["index"] != index { errorsFound <- fmt.Errorf("goroutine %d observed %#v", index, value) }
		}(index)
	}
	wait.Wait()
	close(errorsFound)
	for err := range errorsFound { t.Error(err) }
}
